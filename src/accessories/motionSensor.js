...
class SS3MotionSensor {
    ...
    async startListening() {
        if (this.debug && this.simplisafe.isSocketConnected()) this.log.debug(`${this.name} motion sensor now listening for real time events.`);
        try {
            await this.simplisafe.subscribeToEvents((event, data) => {
                switch (event) {
                    // Socket events
                    case EVENT_TYPES.CONNECTED:
                        if (this.debug) this.log.debug(`${this.name} motion sensor now listening for real time events.`);
                        this.nSocketConnectFailures = 0;
                        break;
                    case EVENT_TYPES.DISCONNECT:
                        if (this.debug) this.log.debug(`${this.name} motion sensor real time events disconnected.`);
                        break;
                    case EVENT_TYPES.CONNECTION_LOST:
                        if (this.debug && this.nSocketConnectFailures == 0) this.log.debug(`${this.name} motion sensor real time events connection lost. Attempting to reconnect...`);
                        setTimeout(async () => {
                ...
                if (data && this.id == data.sensorSerial) {
                    ...
                    if (this.debug) this.log.debug(`${this.name} motion sensor received event: ${event}`);
                    switch (event) {
                        case EVENT_TYPES.MOTION:
                            ...
                            break;
                        default:
                            if (this.debug) this.log.debug(`Motion sensor ${this.id} received unknown event '${event}' with data:`, data);
                            break;
                    }
                }
            });
            ...
            if (err instanceof RateLimitError) {
                ...
                if (this.debug) this.log.debug(`${this.name} motion sensor caught RateLimitError, waiting ${retryInterval/1000}s to retry...`);
                setTimeout(async () => {
                    await this.startListening();
                }, retryInterval);
