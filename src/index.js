...
class SS3Platform {
    ...
    constructor(log, config, api) {
        ...
        this.initialLoad = this.authManager.refreshCredentials()
            ...
            .catch(err => {
                if (err instanceof RateLimitError) {
                    ...
                    this.log.error('SimpliSafe login failed with error:', err.response ? err.response : err);
        ...
        this.api.on('didFinishLaunching', () => {
            ...
            this.initialLoad
                ...
                .catch(err => {
                    this.log.error('Initial accessories refresh failed with error:', err.response ? err.response : err);
                });
        });
    }
        ...
        for (let device of this.devices) {
            ...
            if (!existingAccessory) {
                ...
                try {
                    ...
                    this.log.error('An error occurred while adding accessory:', err.response ? err.response : err);
                }
            }
        }
    }
    ...
    async discoverSimpliSafeDevices() {
        ...
        try {
            ...
            if (err instanceof RateLimitError) {
                this.log.error('Accessory refresh failed due to rate limiting or connectivity:', err.response ? err.response : err);
                this.log.info('Note: this error can also occur if you are not signed up for a SimpliSafe monitoring plan.');
            } else {
                this.log.error('An error occurred while refreshing accessories:', err.response ? err.response : err);
            }
            throw err;
        }
    ...
    async retryBlockedAccessories() {
        try {
            ...
            if (err instanceof RateLimitError) {
                ...
                this.log.error('An error occurred while refreshing credentials again:', err.response ? err.response : err);
            }
        }
    }
