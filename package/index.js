  ...
  constructor(log, config, api) {
    ...
    if (api) {
      ...
      this.api.on('didFinishLaunching', () => {
        ...
        this.simplisafe.login(config.auth.username, config.auth.password, true).then(() => {
          this.log('Logged in!');
  ...
  async refreshAccessories() {
    ...
    try {
      ...
      if (!alarm) {
        ...
        const alarmAccessory = new _alarm.default('SimpliSafe3 Alarm', subscription.location.system.serial, this.log, this.simplisafe, Service, Characteristic, Accessory, UUIDGen);
